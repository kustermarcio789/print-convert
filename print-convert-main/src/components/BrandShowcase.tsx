...
import { getBrands, getProductCountByBrand, getProductsByBrand } from '@/lib/productsData';
      ...
      <div className="container-custom relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {brands.map((brand, index) => {
            const productCount = getProductCountByBrand(brand);
              ...
              >
                <div className="bg-background border border-border rounded-lg p-6 hover:border-accent/50 transition-all hover:shadow-lg h-full flex flex-col">
                  ...
                  <div className="h-20 flex items-center justify-center mb-4 bg-muted rounded-lg overflow-hidden">
                    <img
                      src={brandLogos[brand] || 'https://via.placeholder.com/150x80?text=' + brand}
                      alt={brand}
                  ...
                  {/* Nome da Marca */}
                  <h3 className="text-xl font-bold text-foreground mb-2">{brand}</h3>

                  {/* Contagem de Produtos */}
                  <div className="mb-4 flex-grow">
                    <p className="text-sm text-muted-foreground">
                      <span className="text-2xl font-bold text-accent">{productCount}</span>
                      <span className="ml-2">
                        {productCount === 1 ? 'Produto' : 'Produtos'} disponível{productCount !== 1 ? 's' : ''}
                      </span>
                    </p>
                  </div>
                  ...
                  <Button
                    ...
                    onClick={() => navigate(`/produtos?brand=${brand}`)}
                  >
                    Ver Produtos

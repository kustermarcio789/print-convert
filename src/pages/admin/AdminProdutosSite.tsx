...
import {
  ...
  FileText, UserCheck, Factory, ShoppingCart, Search, Filter, Eye, Trash2, Plus, Image as ImageIcon
} from 'lucide-react';
...
import { produtosSiteAPI } from '@/lib/apiClient'; // Supondo que você tenha uma API para produtos do site

interface ProdutoSite {
  id: string;
  nome: string;
  marca: string;
  categoria: string;
...
export default function AdminProdutosSite() {
  ...
  const [produtosSite, setProdutosSite] = useState<ProdutoSite[]>([]);
  ...
  useEffect(() => {
    const fetchProdutosSite = async () => {
      try {
        setLoading(true);
        const data = await produtosSiteAPI.getAll(); // Buscar todos os produtos do site
        setProdutosSite(data);
      } catch (error) {
        console.error('Erro ao carregar produtos do site:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const filteredProdutosSite = produtosSite.filter((produto) => {
    const matchesSearch =
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm('Tem certeza que deseja excluir este produto do site?')) {
      try {
        await produtosSiteAPI.delete(id);
        setProdutosSite(prev => prev.filter(produto => produto.id !== id));
      } catch (error) {
        console.error(`Erro ao excluir produto do site ${id}:`, error);
      }
    }
  };

  const uniqueBrands = Array.from(new Set(produtosSite.map(p => p.marca)));
  const uniqueCategories = Array.from(new Set(produtosSite.map(p => p.categoria)));
      ...
      <main className="flex-1 overflow-y-auto">
        <header className="bg-white border-b border-gray-200 p-4 sticky top-0 z-10">
          <div className="flex justify-between items-center max-w-7xl mx-auto">
            <h2 className="text-lg font-semibold text-gray-800">Produtos do Site</h2>
        ...
        <div className="p-8 max-w-7xl mx-auto">
          <Card className="mb-6">
            <CardContent className="pt-6">
              <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                <Button onClick={() => navigate('/admin/produtos-site/novo')} className="w-full md:w-auto">
          ...
          {loading ? (
          ...
          ) : filteredProdutosSite.length > 0 ? (
            <div className="space-y-4">
              {filteredProdutosSite.map((produto, index) => (
                <motion.div
                  key={produto.id}
                    ...
                    <CardContent className="p-6">
                      <div className="flex items-start justify-between">
                        <div className="flex items-start gap-4 flex-1">
                          <div className="h-16 w-16 flex-shrink-0 bg-gray-100 rounded overflow-hidden border border-gray-200">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="mt-2 flex flex-wrap gap-2">
                              {produto.active ? (
                                <span className="px-2 py-1 bg-green-100 text-green-800 text-xs font-medium rounded-full">Ativo</span>
                              ) : (
                                <span className="px-2 py-1 bg-red-100 text-red-800 text-xs font-medium rounded-full">Inativo</span>
                              )}
                              {produto.featured && (
                                <span className="px-2 py-1 bg-yellow-100 text-yellow-800 text-xs font-medium rounded-full">Destaque</span>
                              )}
                            </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => navigate(`/admin/produtos-site/${produto.id}`)}
                          >
                          ...
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => navigate(`/admin/produtos-site/${produto.id}/editar`)}
                          >
            ...
            <div className="text-center py-20 bg-white rounded-xl border border-dashed border-gray-200">
              ...
              <p className="text-gray-600 font-medium">Nenhum produto do site encontrado com os filtros selecionados.</p>
              <Button variant="link" onClick={() => { setSearchTerm(''); setFilterBrand('todos'); setFilterCategory('todos'); }} className="mt-2 text-blue-600">

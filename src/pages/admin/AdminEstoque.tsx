...
import { estoqueAPI } from '@/lib/apiClient'; // Supondo que você tenha uma API para estoque
...
interface Material {
  ...
  quantidade: number;
  unidade: string;
  estoque_minimo: number; // Alterado para estoque_minimo
  created_at: string;
}
...
export default function AdminEstoque() {
  ...
  useEffect(() => {
    const fetchMateriais = async () => {
      try {
        setLoading(true);
        const data = await estoqueAPI.getAll(); // Buscar todos os materiais
        setMateriais(data);
      } catch (error) {
      ...
      <main className="flex-1 overflow-y-auto">
        <header className="bg-white border-b border-gray-200 p-4 sticky top-0 z-10">
          <div className="flex justify-between items-center max-w-7xl mx-auto">
            <h2 className="text-lg font-semibold text-gray-800">Estoque</h2>
        ...
        <div className="p-8 max-w-7xl mx-auto">
          ...
          {loading ? (
            ...
            <div className="text-center py-20 bg-white rounded-xl border border-dashed border-gray-200">
              ...
              <p className="text-gray-600 font-medium">Nenhum material em estoque encontrado com os filtros selecionados.</p>
              <Button variant="link" onClick={() => { setSearchTerm(''); setFilterType('todos'); }} className="mt-2 text-blue-600">
